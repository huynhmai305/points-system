...
class Agent extends Component {
  ...
  fetchAuthToken = async uid => {
    const response = await fetch(`http://localhost:3000/chat/api/auth?uid=${uid}`)
    const result = await response.json()
    return result.authToken;
  }

  fetchUsers = async () => {
    const response = await fetch(`http://localhost:3000/chat/api/users`)
    const result = await response.json()
    return result;
  }
				...
				{ 
					customers.map(customer => 
						<li 
							...
							onClick={() => this.props.selectCustomer(customer.uid)}>{customer.name} 
						</li>
					)
				}                
				</ul>
